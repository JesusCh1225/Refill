...
export default function SearchResultPage({ initialQuery, onBack }: SearchResultPageProps) {
  ...
  return (
    <div className="min-h-screen bg-surface-page text-text-body">
      ...
      <button onClick={handleWriteClick} className="fab-above-nav fixed right-6 z-10 flex items-center gap-2 bg-brand text-white text-xs font-semibold px-4 rounded-full border-none cursor-pointer hover:opacity-85 transition-opacity shadow-search" style={{ height: "44px" }}>
        ✦ 글쓰기
      </button>
